    ...
    >
      <div
        ...
        className={`relative w-full max-w-md rounded-2xl shadow-2xl border overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-150 ${
          isDark ? 'bg-[#0d0d12] border-purple-500/30 text-white' : 'bg-white border-neutral-200 text-neutral-900'
        }`}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className={`px-4 py-3 border-b flex items-center justify-between sticky top-0 z-10 ${
          isDark ? 'bg-[#111116] border-purple-500/25' : 'bg-white border-neutral-200'
        }`}>
          <div className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center border ${
              isDark ? 'bg-purple-900/40 border-purple-500/30 text-slate-200' : 'bg-purple-100 border-purple-200 text-purple-700'
            }`}>
            ...
            <div>
              ...
              <p className={`text-[11px] font-mono ${isDark ? 'text-slate-300' : 'text-neutral-500'}`}>{currentUser.handle}</p>
            </div>
          </div>
          ...
          <button
            ...
            className={`w-7 h-7 rounded-full flex items-center justify-center transition cursor-pointer ${
              isDark ? 'text-slate-200 hover:text-white hover:bg-purple-900/40' : 'text-neutral-400 hover:text-neutral-700 hover:bg-neutral-100'
            }`}
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <div className={`flex border-b ${isDark ? 'border-purple-900/40 bg-[#000000]' : 'border-neutral-200 bg-neutral-50/50'}`}>
          <button
            type="button"
            ...
            className={`flex-1 py-2.5 text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer border-b-2 ${
              activeTab === 'following'
                ? isDark
                  ? 'border-purple-400 text-slate-200 bg-[#14141b]'
          ...
          >
            ...
            <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-mono ${
              isDark ? 'bg-purple-950/80 text-slate-200 border border-purple-700/40' : 'bg-neutral-200/80 text-neutral-700'
            }`}>
        ...
        {/* Search filter input */}
        <div className={`p-3 border-b ${isDark ? 'border-purple-900/30 bg-[#0a0a0f]' : 'border-neutral-100 bg-white'}`}>
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs border ${
            isDark ? 'bg-[#171720] border-purple-800/40 text-white' : 'bg-neutral-100 border-neutral-200 text-neutral-700'
          }`}>
            <Search className={`w-3.5 h-3.5 shrink-0 ${isDark ? 'text-slate-300' : 'text-neutral-400'}`} />
            <input
              type="text"
              ...
              className={`w-full bg-transparent outline-none text-xs placeholder:text-slate-400 ${
                isDark ? 'text-white placeholder:text-slate-400' : 'text-neutral-900 placeholder:text-neutral-400'
              }`}
            />
            {searchQuery && (
              <button
                ...
                className={isDark ? 'text-slate-200 hover:text-white' : 'text-neutral-400 hover:text-neutral-600'}
              >
                ✕
              </button>
            )}
        ...
        {activeTab === 'followers' && (
          <div className={`px-3 py-2.5 border-b text-xs ${
            isDark
              ? 'bg-[#171720] border-purple-900/40 text-slate-200'
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className={`font-bold text-[11px] ${isDark ? 'text-slate-200' : 'text-neutral-800'}`}>
            ...
            {usersWhoCanFollowMe.length > 0 ? (
              <div className="mt-1.5 flex items-center gap-1 overflow-x-auto no-scrollbar">
                <span className={`text-[10px] shrink-0 ${isDark ? 'text-slate-300' : 'text-neutral-500'}`}>Click to follow you:</span>
                {usersWhoCanFollowMe.map((u) => (
                  <button
                    ...
                    className={`text-[10px] px-2 py-0.5 rounded-full transition shrink-0 cursor-pointer font-medium border ${
                      isDark
                        ? 'bg-purple-950/70 hover:bg-purple-900/80 border-purple-700/50 text-slate-200'
              ...
              <p className={`text-[10px] mt-1 ${isDark ? 'text-slate-200' : 'text-purple-700'}`}>
        ...
        <div className={`flex-1 overflow-y-auto divide-y p-2 ${
          isDark ? 'divide-purple-900/30 bg-[#000000]' : 'divide-neutral-100 bg-white'
        }`}>
          {currentDisplayedList.length === 0 ? (
            <div className="p-8 text-center text-neutral-400">
              <Users className={`w-8 h-8 mx-auto mb-2 ${isDark ? 'text-purple-500/40' : 'text-neutral-300'}`} />
              <p className={`text-xs font-semibold ${isDark ? 'text-slate-200' : 'text-neutral-600'}`}>
                {activeTab === 'following'
              ...
              <p className={`text-[11px] mt-1 ${isDark ? 'text-slate-400' : 'text-neutral-400'}`}>
                {activeTab === 'following'
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-xl transition flex items-start justify-between gap-3 ${
                    isDark ? 'hover:bg-[#131319]' : 'hover:bg-neutral-50/80'
                  }`}
                >
                  {/* Avatar & User Info */}
                  <div className="flex items-start gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {user.isFollowingMe && (
                          <span className={`text-[9px] font-semibold px-1.5 py-0.2 rounded-md border ${
                            isDark
                              ? 'bg-purple-950/80 text-slate-200 border-purple-700/40'
                      ...
                      <p className={`text-[11px] font-mono ${isDark ? 'text-slate-300' : 'text-neutral-500'}`}>{user.handle}</p>
                      <p className={`text-[11px] mt-1 line-clamp-2 leading-tight ${isDark ? 'text-slate-200' : 'text-neutral-600'}`}>
                        {user.bio}
                      </p>
                    </div>
                  </div>
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-bold transition cursor-pointer border ${
                          isHovered
                            ...
                            : isDark
                            ? 'bg-[#1c1c26] text-slate-200 border-purple-500/40 hover:bg-[#25124b]'
        ...
        <div className={`px-4 py-2.5 border-t text-center text-[10px] ${
          isDark ? 'bg-[#0a0a0f] border-purple-900/30 text-slate-300' : 'bg-neutral-50 border-neutral-200 text-neutral-500'
        }`}>
