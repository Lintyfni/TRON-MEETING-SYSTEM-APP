...
export const GroupMessengerChat: React.FC<GroupMessengerChatProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-neutral-50/50">
      {/* 1. Group Selector Carousel */}
      <div className="px-4 py-2 bg-white border-b border-neutral-200 flex items-center gap-2 overflow-x-auto scrollbar-none shrink-0">
        ...
        {groups.map((grp) => {
          ...
          const pendingCount = grp.pendingRequests.length;
          ...
          return (
            <button
              ...
              className={`h-8 px-3 rounded-xl text-xs font-bold transition flex items-center gap-2 shrink-0 cursor-pointer ${
                isSelected
            ...
            >
              ...
              />
              <span className="truncate max-w-[140px]">{grp.name}</span>
              {pendingCount > 0 && (
        ...
        <button
          type="button"
          onClick={onCreateGroupClick}
          className="h-8 px-3 rounded-xl text-xs font-bold bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200 flex items-center gap-1.5 shrink-0 cursor-pointer transition shadow-2xs"
      ...
      {/* 2. Group Header */}
      <div className="px-4 py-2 bg-white border-b border-neutral-200 flex items-center justify-between gap-3 shrink-0 shadow-2xs">
        ...
        {/* Action Controls - Standardized Matching Size */}
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          {activeGroup.linkedMeetingToken && onJumpToMeeting && (
            <button
              type="button"
              onClick={() => onJumpToMeeting(activeGroup.linkedMeetingToken!)}
              className="h-8 px-2.5 rounded-xl bg-neutral-100 hover:bg-neutral-200 text-neutral-800 border border-neutral-200 text-xs font-semibold transition flex items-center gap-1.5 cursor-pointer shrink-0"
              title={`Join Group Meeting (${activeGroup.linkedMeetingToken})`}
            >
          ...
          {onGoToGroupPosts && (
            <button
              type="button"
              onClick={() => onGoToGroupPosts(activeGroup.id)}
              className="h-8 px-2.5 rounded-xl bg-neutral-100 hover:bg-neutral-200 text-neutral-800 border border-neutral-200 text-xs font-semibold transition flex items-center gap-1.5 cursor-pointer shrink-0"
          ...
          >
            ...
            {activeGroup.pendingRequests.length > 0 && (
              <span className="w-4 h-4 bg-red-500 text-white rounded-full text-[9px] font-extrabold flex items-center justify-center">
